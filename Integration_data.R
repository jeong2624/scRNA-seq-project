...
Load_packages =  c("dplyr","Seurat","patchwork","readr", "stringr")
...
DoHeatmap(data.combined, features = top5$gene)
...
DotPlot(data.combined, features = markers.to.plot, cols = c("blue", "red"), 
...
FeaturePlot(data.combined, features = c("ARID1B", "ASH1L"), 
            split.by = "orig.ident", min.cutoff = "q9",
            cols = c("grey", "red"))

### Identify differential expressed genes across conditions
cortex.markers <- FindConservedMarkers(data.combined, ident.1 = c("Excitatory neurons"),
                                       ident.2 = c("Inhibitory neurons"),
                                       grouping.var = "orig.ident", verbose = FALSE)
head(cortex.markers)

### visualizing marker expression. (violin plot)
plots1 <- VlnPlot(data.combined, features = c("ARID1B", "ASH1L", "CHD2", "AFF2"), 
                  split.by = "orig.ident", group.by = "group",
                  pt.size = 0, combine = FALSE, split.plot = TRUE)
wrap_plots(plots = plots1, ncol = 2)

plots2 <- VlnPlot(data.combined, features = c("MECP2", "CACNA1C", "SHANK2", "ATRX"), 
                  split.by = "orig.ident", group.by = "group",
                  pt.size = 0, combine = FALSE, split.plot = TRUE)
wrap_plots(plots = plots2, ncol = 2)
